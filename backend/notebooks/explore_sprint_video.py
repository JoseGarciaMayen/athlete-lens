...
VIDEO_PATH = "tests/core/sprint/fixtures/sprint.mp4"  # adjust if needed
MODEL_PATH = "yolov8n-pose.pt"
OUTPUT_DIR = "notebooks/sprint_explore_output"
# How many frames from the end to analyse (athlete appears late in video)
ANALYSE_LAST_N_FRAMES = 1000
...
for f in range(start_frame, total):
    ...
    for r in results:
        ...
        # Hip keypoints (11=left hip, 12=right hip) for a more stable X reference
        hip_cx = None
...
for d in detections:
    ...
    print(f"{d['frame']:>6} | {d['time_s']:>7.3f} | {d['cx']:>7.1f} | {hip_str} | {d['conf']:>6.3f} | {bbox_flag:<8} | {hip_flag}")
...
print(f"\nVideo duration (= sprint time if T0 = frame 0): {duration:.3f}s")

print("\n[Phase 4] Saving annotated frames around crossing...")

save_target = first_hip_crossing or first_bbox_crossing
if save_target:
    around = range(max(0, save_target['frame'] - 10), min(total, save_target['frame'] + 10))
    for f in around:
        cap.set(cv2.CAP_PROP_POS_FRAMES, f)
        ret, frame = cap.read()
        if not ret:
        ...
        cv2.putText(annotated, f"frame {f} | mid={midpoint_x:.0f}px",
